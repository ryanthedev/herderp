...
Read a Claude Code session the user has lost track of, in place — get up to speed on what it did without relaunching it. Seven MCP tools back this: `necromancy_resolve` (turn a herdr handle like `upublish:1` into an exact session), `necromancy_find_spaces`, `necromancy_list_sessions` (resolve and enumerate sessions), `necromancy_anchors` (the deterministic "always grab" set), and `necromancy_outline`, `necromancy_search`, `necromancy_read` (the reading path) — this skill is the conversation that drives them in the right order and reads their results honestly.
